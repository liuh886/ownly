...
# ORS optimized stops are Visit projections; only persisted leg endpoints use canonical place ids.
p = Path('scripts/mcp/openrouteservice.ts')
s = p.read_text()
s = s.replace('    const from: PlannerTripPlace = result.places[index];', '    const from: PlannerScheduledPlace = result.places[index];')
s = s.replace('    const to: PlannerTripPlace = result.places[index + 1];', '    const to: PlannerScheduledPlace = result.places[index + 1];')
p.write_text(s)

# Travel-time optimizer tests use Visit identity for matrix/order and canonical place identity separately.
Path('src/domain/planner-route-time.test.ts').write_text(r'''import { describe, expect, it } from 'vitest';
...
print('visit route-time typing staged')
