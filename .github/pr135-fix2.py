...
# Travel-time optimization operates on Visit projections, not reusable Place records.
p = Path('src/domain/planner-route-time.ts')
s = p.read_text()
s = s.replace("import type { PlannerTripPlace } from './planner';", "import type { PlannerScheduledPlace } from './planner-visits';")
s = s.replace('PlannerTripPlace[]', 'PlannerScheduledPlace[]')
s = s.replace('PlannerTripPlace>', 'PlannerScheduledPlace>')
p.write_text(s)

# Core + finalize both guarded iCal inputs during iterative staging; collapse duplicate guard declarations/assertions.
p = Path('scripts/shared/ownly-write-service.ts')
s = p.read_text()
dup_decl = "    const expectedVisits = new Map(visitEntries.map((entry) => [entry.filePath, fingerprint(entry.filePath)] as const));\n    const expectedVisits = new Map(visitEntries.map((entry) => [entry.filePath, fingerprint(entry.filePath)] as const));\n"
s = s.replace(dup_decl, "    const expectedVisits = new Map(visitEntries.map((entry) => [entry.filePath, fingerprint(entry.filePath)] as const));\n")
dup_assert = "        for (const entry of visitEntries) assertUnchanged(entry.filePath, expectedVisits.get(entry.filePath)!);\n        for (const entry of visitEntries) assertUnchanged(entry.filePath, expectedVisits.get(entry.filePath)!);\n"
s = s.replace(dup_assert, "        for (const entry of visitEntries) assertUnchanged(entry.filePath, expectedVisits.get(entry.filePath)!);\n")
p.write_text(s)

# Schedule tests model repeatable occurrences explicitly.
Path('src/domain/planner-schedule.test.ts').write_text(r'''import { describe, expect, it } from 'vitest';
...
print('visit typing and schedule tests staged')
