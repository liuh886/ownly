...
External captures are instantly written to the active Inbox collection in `ownlyCaptureStateV3` with provider namespace isolation (e.g. `agoda`, `booking`, `tabelog`, `xiaohongshu`, `google_travel`), and asynchronously enriched by the background worker via Google Maps query resolution (`resolveAndEnrichCapturedPlace`), adding canonical Place IDs (`0x...:0x...`), coordinates, verified ratings, opening hours, and hotel property facts.
...
Places are de-duplicated per collection using **provider-native identity based merge** for strong identities (matching provider-native ID or Google Place ID/CID and exact canonical URLs). Weak signals (title similarity, geographic proximity) are strictly categorized as **weak evidence based duplicate suggestion** for user review and never trigger automatic merging. Re-importing a saved list via one-click sync refreshes captured observations while preserving the user's Planner-owned research edits; bulk text/link paste only adds missing places and never overwrites existing ones.
