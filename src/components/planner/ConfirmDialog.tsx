...
export function ConfirmDialog({ zh, title, message, confirmLabel, onConfirm, onClose }: ConfirmDialogProps) {
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-stone-950/60 p-4 backdrop-blur-xs ownly-fade-in">
      <div
        className="w-full max-w-sm rounded-2xl border border-stone-200 bg-white p-5 shadow-2xl space-y-4 ownly-pop-in"
        role="alertdialog"
        aria-modal="true"
        aria-label={title}
      >
