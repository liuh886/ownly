...
const copy = {
  en: {
    ...
    trusted: ['No required account', 'Plain Markdown', 'Backup and restore', 'Open source'],
    mockup: {
      label: 'Your ownership, made legible',
      annual: 'Annual recurring cost',
      annualValue: '$1,284',
      objects: 'Active possessions',
      objectsValue: '42',
      reviews: 'Reviews due',
      reviewsValue: '3',
      insight: 'Decision cue',
      insightText: 'The camera has been used twice in six months. Review before buying another lens.',
      itemA: 'Noise-cancelling headphones',
      itemAMeta: 'Used 18 times · worth keeping',
      itemB: 'Cloud storage plan',
      itemBMeta: '$119.88 / year · review in 12 days',
    },
    ...
    values: [
      {
        number: '01',
        title: 'Remember what entered your life',
        text: 'Record physical items, subscriptions, plans and experiences in one durable, human-readable system.',
      },
      {
        number: '02',
        title: 'Understand the real cost',
        text: 'Bring price, recurring expense, use, condition and time into the same ownership picture.',
      },
      {
        number: '03',
        title: 'Review before the next decision',
        text: 'Use real experience to decide whether to keep, renew, replace, transfer or let go.',
      },
    ],
    ...
    workflow: [
      ...
      ['Review', 'Compare cost and experience at the moment a decision is due.'],
    ...
    architecture: {
      app: 'Ownly Web / PWA',
      folder: 'Local Ownly data folder',
    ...
    platforms: [
      {
        title: 'Web / PWA',
        tag: 'Start here',
        text: 'Use Ownly directly in a current desktop Chrome or Edge browser, then install it as a focused standalone app.',
      },
      {
        title: 'Obsidian plugin',
        tag: 'Vault native',
        text: 'Work with the same records inside an Obsidian Vault while retaining direct Markdown access.',
      },
      {
        title: 'Agent CLI',
        tag: 'Fact ready',
        text: 'Use deterministic JSON commands for scripts, automation and external agents without turning Ownly into an AI assistant.',
      },
    ],
  ...
  zh: {
    ...
    openApp: '打开 Ownly',
    preview: '查看产品预览',
    trusted: ['无需注册账户', '纯 Markdown', '备份与恢复', '开源'],
    mockup: {
      label: '让所有权变得清晰',
      annual: '年度持续支出',
      annualValue: '¥9,240',
      ...
      reviewsValue: '3',
      insight: '决策提示',
      insightText: '这台相机半年只使用了两次。购买下一支镜头前，先完成一次回顾。',
      itemA: '降噪耳机',
      itemAMeta: '已使用 18 次 · 值得保留',
      itemB: '云存储订阅',
      itemBMeta: '¥828 / 年 · 12 天后回顾',
    },
    ...
    values: [
      {
        number: '01',
        title: '记住什么进入了你的生活',
        text: '用一个持久、可读的系统记录物品、订阅、计划和重要经历。',
      },
      {
        number: '02',
        title: '理解真实成本',
        text: '把价格、持续支出、使用频率、状态和时间放在同一张所有权图景中。',
      },
      {
        number: '03',
        title: '在下一次决定前回顾',
        text: '根据真实经历判断保留、续费、更换、转让或放弃。',
      },
    ],
    ...
    platforms: [
      {
        title: 'Web / PWA',
        tag: '推荐入口',
        text: '在当前桌面版 Chrome 或 Edge 中直接使用，也可以安装为专注、独立的应用窗口。',
      },
      {
        title: 'Obsidian 插件',
        tag: '原生 Vault',
        text: '在 Obsidian Vault 中使用同一批记录，并保留直接查看和编辑 Markdown 的能力。',
      },
      {
        title: 'Agent CLI',
        tag: '事实就绪',
        text: '通过确定性的 JSON 命令支持脚本、自动化和外部 Agent，但 Ownly 本身不是 AI 助手。',
      },
    ],
...
export function MarketingHome({ appHref, githubHref, obsidianHref }: MarketingHomeProps) {
  ...
  useEffect(() => {
    if (navigator.language.toLowerCase().startsWith('zh')) setLanguage('zh');
  }, []);
    ...
    <main className="min-h-screen overflow-hidden bg-[#f4f1e9] text-stone-950">
      <header className="sticky top-0 z-40 border-b border-stone-900/8 bg-[#f4f1e9]/88 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8 lg:px-10">
          ...
          <nav className="hidden items-center gap-7 text-sm text-stone-600 lg:flex" aria-label="Primary navigation">
            <a className="transition hover:text-stone-950" href="#why">{text.nav.features}</a>
            <a className="transition hover:text-stone-950" href="#workflow">{text.nav.workflow}</a>
            <a className="transition hover:text-stone-950" href="#local-first">{text.nav.privacy}</a>
            <a className="transition hover:text-stone-950" href="#platforms">{text.nav.platforms}</a>
          </nav>
            ...
            <div className="overflow-hidden rounded-[1.45rem] bg-[#f8f6f0]">
              <div className="flex items-center justify-between border-b border-stone-200 px-5 py-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-sm font-semibold">Ownly</p>
                    <p className="text-[10px] text-stone-500">{text.mockup.label}</p>
                  </div>
                </div>
              ...
              <div className="grid gap-3 p-4 sm:grid-cols-3 sm:p-5">
                {[
                  [text.mockup.annual, text.mockup.annualValue],
                  [text.mockup.objects, text.mockup.objectsValue],
                  [text.mockup.reviews, text.mockup.reviewsValue],
                ].map(([label, value]) => (
              ...
              <div className="grid gap-4 px-4 pb-4 sm:grid-cols-[1.1fr_0.9fr] sm:px-5 sm:pb-5">
                <div className="rounded-2xl border border-stone-200 bg-white p-4">
                  <div className="mb-4 flex items-center justify-between">
                    <p className="text-xs font-semibold">Recent ownership facts</p>
                    <span className="text-[10px] text-stone-400">Markdown</span>
                  </div>
                  <div className="space-y-3">
                    <div className="rounded-xl bg-stone-50 p-3">
                      <p className="text-xs font-semibold">{text.mockup.itemA}</p>
                      <p className="mt-1 text-[10px] leading-4 text-stone-500">{text.mockup.itemAMeta}</p>
                    </div>
                    <div className="rounded-xl bg-stone-50 p-3">
                      <p className="text-xs font-semibold">{text.mockup.itemB}</p>
                      <p className="mt-1 text-[10px] leading-4 text-stone-500">{text.mockup.itemBMeta}</p>
                    </div>
                  </div>
                </div>
                <div className="rounded-2xl bg-emerald-900 p-4 text-white">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-emerald-200">{text.mockup.insight}</p>
                  <p className="mt-5 text-sm font-medium leading-6">{text.mockup.insightText}</p>
      ...
      <section id="why" className="border-y border-stone-900/8 bg-white/45">
        <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-10 lg:py-28">
          <div className="max-w-3xl">
            <h2 className="text-4xl font-semibold tracking-[-0.05em] sm:text-5xl">{text.valueTitle}</h2>
            <p className="mt-5 text-base leading-7 text-stone-600 sm:text-lg sm:leading-8">{text.valueIntro}</p>
          </div>
          <div className="mt-12 grid gap-4 lg:grid-cols-3">
            {text.values.map((item) => (
              <article key={item.number} className="rounded-3xl border border-stone-900/10 bg-[#f8f6f0] p-7 sm:p-8">
                <p className="text-xs font-semibold tracking-[0.18em] text-emerald-800">{item.number}</p>
      ...
      <section id="workflow" className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-10 lg:py-28">
        <div className="grid gap-12 lg:grid-cols-[0.7fr_1.3fr]">
          <div>
            <h2 className="text-4xl font-semibold tracking-[-0.05em] sm:text-5xl">{text.workflowTitle}</h2>
            <p className="mt-5 max-w-xl text-base leading-7 text-stone-600">{text.workflowIntro}</p>
          </div>
          ...
          <div>
            ...
            <div className="mt-8 space-y-3">
              {text.localPoints.map((point) => (
                <div key={point} className="flex gap-3 text-sm leading-6 text-stone-200"><CheckIcon />{point}</div>
              ))}
            </div>
          </div>

          <div className="rounded-[2rem] border border-white/12 bg-white/[0.04] p-5 sm:p-7">
            <div className="rounded-2xl border border-emerald-300/25 bg-emerald-300/10 px-5 py-4 text-center text-sm font-semibold text-emerald-100">
              {text.architecture.app}
            </div>
            <div className="mx-auto h-10 w-px bg-white/20" />
            <div className="rounded-2xl border border-white/15 bg-white/[0.06] p-5 text-center">
              <p className="text-sm font-semibold">{text.architecture.folder}</p>
              <p className="mt-1 text-xs text-stone-400">{text.architecture.markdown}</p>
            </div>
            <div className="grid grid-cols-2 gap-4 pt-10 before:absolute">
              <div className="rounded-2xl border border-white/12 bg-white/[0.04] px-5 py-4 text-center text-sm text-stone-200">{text.architecture.obsidian}</div>
              <div className="rounded-2xl border border-white/12 bg-white/[0.04] px-5 py-4 text-center text-sm text-stone-200">{text.architecture.cli}</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="platforms" className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-10 lg:py-28">
        <div className="max-w-3xl">
          <h2 className="text-4xl font-semibold tracking-[-0.05em] sm:text-5xl">{text.platformsTitle}</h2>
          <p className="mt-5 text-base leading-7 text-stone-600">{text.platformsIntro}</p>
        </div>
        <div className="mt-12 grid gap-4 lg:grid-cols-3">
          {text.platforms.map((platform) => (
            <article key={platform.title} className="flex min-h-64 flex-col rounded-3xl border border-stone-900/10 bg-white/65 p-7 sm:p-8">
              <span className="w-fit rounded-full bg-stone-950 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-white">{platform.tag}</span>
              <h3 className="mt-8 text-2xl font-semibold tracking-[-0.035em]">{platform.title}</h3>
              <p className="mt-4 text-sm leading-6 text-stone-600">{platform.text}</p>
              <div className="mt-auto pt-8">
                {platform.title.includes('Obsidian') ? (
                  <a href={obsidianHref} onClick={() => trackPublicCta('platform_obsidian')} className="inline-flex items-center gap-2 text-sm font-semibold hover:text-emerald-800">{text.obsidian}<ArrowIcon /></a>
                ) : platform.title.includes('Web') ? (
                  <a href={appHref} onClick={() => trackPublicCta('platform_web')} className="inline-flex items-center gap-2 text-sm font-semibold hover:text-emerald-800">{text.openApp}<ArrowIcon /></a>
                ) : (
                  <a href={githubHref} onClick={() => trackPublicCta('platform_github')} className="inline-flex items-center gap-2 text-sm font-semibold hover:text-emerald-800">{text.github}<ArrowIcon /></a>
                )}
              </div>
            </article>
          ))}
        ...
        <div className="mt-5 rounded-3xl border border-amber-900/12 bg-amber-50/70 p-7 sm:p-8">
          <h3 className="text-lg font-semibold">{text.boundariesTitle}</h3>
          <p className="mt-3 max-w-4xl text-sm leading-6 text-stone-600">{text.boundariesText}</p>
        </div>
      </section>
      ...
      <section className="px-5 pb-20 sm:px-8 lg:px-10 lg:pb-28">
        <div className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] bg-emerald-900 px-7 py-14 text-white sm:px-12 sm:py-16 lg:flex lg:items-end lg:justify-between lg:px-16">
          <div className="max-w-3xl">
            <h2 className="text-4xl font-semibold tracking-[-0.05em] sm:text-5xl">{text.finalTitle}</h2>
            <p className="mt-5 text-base leading-7 text-emerald-100">{text.finalText}</p>
          </div>
          <a
            href={appHref}
            onClick={() => trackPublicCta('final_open_app')}
            className="mt-8 inline-flex shrink-0 items-center gap-2 rounded-full bg-white px-6 py-3.5 text-sm font-semibold text-emerald-950 transition hover:-translate-y-0.5 lg:mt-0"
          >
            {text.openApp}
            <ArrowIcon />
          </a>
      ...
      <footer className="border-t border-stone-900/8">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 px-5 py-8 text-sm text-stone-500 sm:flex-row sm:items-center sm:justify-between sm:px-8 lg:px-10">
          <div>
            <span className="font-semibold text-stone-900">Ownly</span>
            <span className="ml-3">{text.footer}</span>
          </div>
          <div className="flex items-center gap-5">
            <a className="transition hover:text-stone-950" href={githubHref}>{text.github}</a>
            <a className="transition hover:text-stone-950" href={obsidianHref}>{text.obsidian}</a>
            <span>© {year}</span>
          </div>
        </div>
      </footer>
