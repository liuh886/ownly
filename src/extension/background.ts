  ...
  try {
    ...
    await mutateCaptureStateV3InWorker((currentState) => {
      ...
      const updatedPlace: CapturePlace = {
        ...
        service_options: enriched.service_options?.length ? enriched.service_options : existingPlace.service_options,
  ...
  try {
    ...
    const capturedId = await mutateCaptureStateV3InWorker((state) => {
      ...
      const capturePlace: CapturePlace = {
        ...
        service_options: place.serviceOptions?.length ? place.serviceOptions : existing?.service_options,
      ...
      const validIncoming = incoming.places
        ...
        .map((incomingPlace) => {
          ...
          return {
            ...
            service_options: incomingPlace.service_options?.length ? incomingPlace.service_options : existing.service_options,
    ...
    void mutateCaptureStateV3InWorker((current) => {
      ...
      if (idx === -1) {
        ...
        const merged: CapturePlace = {
          ...
          service_options: place.service_options?.length ? place.service_options : existing.service_options,
